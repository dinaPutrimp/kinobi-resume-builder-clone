    ...
    return (
        <div className="p-3 shadow-t-side rounded-lg bg-white">
            ...
            <div className="accordian" id="accordianCard">
                {resumeState && resumeState.currentResume && resumeState.currentResume.organizations.map((organization, index) => {
                    return (
                        <div key={index} className="accordion-item shadow bg-white mb-4 md:px-4 md:mb-3" id={`heading${index}`} onDragStart={(e) => handleDragStart(e, index)} onDragEnter={(e) => handleDragEnter(e, index)} onDragEnd={handleDropCard} draggable>
                            <label className="flex items-center justify-between accordion-button py-4 px-5" data-bs-toggle="collapse" data-bs-target={`#collapse${index}`} aria-expanded="false" aria-controls={`collapse${index}`}>
                                <div className="flex items-center">
                                    ...
                                    <i className="fa fa-trash text-xl text-red-500 cursor-pointer" onClick={() => dispatchResume({ type: "REMOVE_FORM_ORGANIZATION", payload: { index: index } })}></i>
                                </div>
                            </label>
                                    ...
                                    <div>
                                        ...
                                        <ContentEditable
                                            ...
                                            onChange={(e) => dispatchResume({
                                                type: "UPDATE_ORGANIZATION",
                                                payload: { name: "description", value: e.target.value, index: index }
            ...
            <div className="flex flex-row justify-end mb-4">
                <Link to={`/resume/${location.pathname.split('/').at(2)}/education`} className="block w-auto py-1.5 px-4 bg-white border-2 border-blue-900 text-blue-800 rounded uppercase font-medium self-end mr-3">Back</Link>
                <Link to={`/resume/${location.pathname.split('/').at(2)}/others`} onClick={() => handleOrganisationsUpdate(resumeState.currentResume)} className="block w-auto py-2 px-4 bg-blue-900 text-white rounded uppercase font-medium">Save & Continue</Link>
            </div>
        </div>
    );
}
